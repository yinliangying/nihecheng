...
class MyReactionToken(translate_source_features.SourceFeatureProblem):
    ...
    def get_or_create_vocab(self, data_dir, tmp_dir, force_get=False, file_byte_budget=1e6):
        ...
        if self.vocab_type == VocabType.TOKEN:
            vocab_filename = os.path.join(data_dir, self.vocab_filename)
            encoder = text_encoder.TokenTextEncoder(vocab_filename,
                                                    replace_oov=self.oov_token)
            return encoder
        else:
            return translate_source_features.SourceFeatureProblem.get_or_create_vocab(self,data_dir, tmp_dir, force_get, file_byte_budget)
