...
fp_train_features=open(os.path.join(onepath, mode+"_features"), "w")
fp_test_id=open(os.path.join(onepath, mode+"_id"), "w")
...
def process_trainset():
    for line in fp_train:
        ...
        out_feature_list=[]
        for tmp in out_product_list:
            out_feature_list.append("%s|%s"%(random.randint(1,10),random.randint(10,20)))
        print(" ".join(out_feature_list), file=fp_train_features)
